...
2. **They are type-safe.** Using Typescript I can define what values are permitted for each style prop and enforce valid
   inputs with type checking. With Inellisense I can also provide hints and auto-completion to make references easier
   and reduce confusion. Otherwise, normal CSS or styled components are written as plain text, which cannot be easily
   parsed to check for proper types or syntax.
